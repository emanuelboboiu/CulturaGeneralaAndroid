...
public class SetsManagement {
	...
	private final int BASE_VIEW_ID = 1000010000; // hope not interfere.
	private TextView[] arrTV = null;
	private ArrayList<String> arrSets = null;
	private boolean isAllSets = false;
	private int nrSets = 0;
	private int lastId = 0; // for last item unselect all.
	...
	public SetsManagement(Context context) {
		...
		for (int i = 0; i < curSetIds.length; i++) {
			arrSets.add(curSetIds[i]);
		} // end for.
	...
	public void showSets() {
		...
		do {
			...
			tv.setOnClickListener(new OnClickListener() {
				@Override
				public void onClick(View view) {
					afterClickAnItem(setId, tempIndex);
				}
			});
			// End add listener for short click on a set item.
		...
		tv.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View view) {
				afterClickAnItem(lastId, lastId);
			}
		});
	...
	private int arrSetsContainsSetId(String setId) {
		...
		for (int i = 0; i < arrSets.size(); i++) {
			if (arrSets.get(i).toString().equals(setId)) {
				exists = i;
				break;
	...
	private String getSetAuthor(int setId) {
		...
		cursor.moveToFirst();
		String authorName = cursor.getString(0);
		cursor = null;
