...
public class DataBaseHelper extends SQLiteOpenHelper {
	...
	private static int lastUpdateTimestamp = 1637510400; // 20 November 2021.
	...
	private void saveDatabaseUsedQuestions(int oldDvVersion) {
		...
		DbOpenAndReadMain mDbHelper = new DbOpenAndReadMain(mContext, DB_NAME,
				null, oldDvVersion);
	...
	private void setUsedFieldValuesBack() {
		...
		if (cursor.getCount() > 0) {
			...
			cursor.moveToFirst();
			do {
				m1.updateData("UPDATE intrebari SET consumat=1 WHERE intrebareId="
						+ cursor.getInt(0));
			} while (cursor.moveToNext());
