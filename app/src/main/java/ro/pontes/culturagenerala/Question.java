...
public class Question {
	...
	private LinearLayout llVariants = null;
	...
	private void generate(boolean isNew) {
		...
		for (int i = 0; i < 4; i++) {
			...
			variants[i] = new String(cursor.getString(i + 5));
	...
	private void getTextViews() {
		...
		tvQuestion = (TextView) activity.findViewById(R.id.tvQuestion);
		...
		for (int i = 0; i < 4; i++) {
			...
			tvVariants[i] = (TextView) activity.findViewById(resID);
		} // end for.
		...
		for (int i = 0; i < 4; i++) {
			...
			tvLetters[i] = (TextView) activity.findViewById(resID);
		} // end for.

		// Now work at resize in a tree observer:
		llVariants = (LinearLayout) activity.findViewById(R.id.llVariants);
				...
				new ViewTreeObserver.OnGlobalLayoutListener() {
					...
					public void onGlobalLayout() {
						if (!isVariantsResized) {
							...
							LinearLayout llVariants = (LinearLayout) activity
									.findViewById(R.id.llVariants);
							int llVariantsWidth = llVariants.getWidth();
							...
							paramsForLetters.width = tvLettersWidth;
	...
	public void enableOrDisableAllVariants(boolean status) {
		for (int i = 0; i < 4; i++) {
			...
			if (status) {
				tvVariants[i].setFocusable(true);
			} else {
				tvVariants[i].setFocusable(false);
			}
		} // end for.
	...
	public String callFriend() {
		...
		String msgToReturn = String.format(
	...
	public String askPublic() {
		...
		String rezPublic = ""; // a variable with public vote.
		...
		for (int i = 0; i < aProcVariants.length; i++) {
			rezPublic += aABCD[i] + " = " + aProcVariants[i] + "%, ";
		} // end for.
			// Cut the final comma:
		rezPublic = rezPublic.substring(0, rezPublic.length() - 2);
		String msgToReturn = String.format(
