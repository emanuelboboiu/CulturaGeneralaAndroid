...
public class Quiz {
	...
	// Fields:
	private int delayToNextQuestion = 3500;
	...
	private final int CHRON_RED_SECOND = 15;
	private int cumulativeTime = 0;
	private int TOTAL_TIME = 101; // First second is never shown.
	...
	private boolean[] ibStatus = { true, true, true, false, true };
	...
	private void beforeStart() {
		...
		layoutMain = (RelativeLayout) activity.findViewById(R.id.layoutMain);
		...
		llForIbsLayout = (LinearLayout) activity
				.findViewById(R.id.llForIbsLayout);

		// Charge the chronometer:
		chron = (TextView) activity.findViewById(R.id.tvGameTimer);
					...
					public void onGlobalLayout() {
						...
						if (chronBackground == null) {
							...
							if (MainActivity.isPortrait) {
								 ...
								 */
								LinearLayout llChron = (LinearLayout) activity
						...
						if (!areVariantsPositionedInMiddle
							...
							handler.postDelayed(new Runnable() {
								...
								public void run() {
									...
									LinearLayout llFirstVariant = (LinearLayout) activity
									 ...
									 */
									ScrollView svVariants = (ScrollView) activity
		...
		for (int i = 0; i < 21; i++) {
			...
			tvPBs[i] = (TextView) activity.findViewById(resID);
		} // end for.
	...
	public void showHelperButtons() {
		...
		for (int i = 0; i < ibOptions.length; i++) {
			...
			ibOptions[i] = (ImageButton) activity.findViewById(resID);
		} // end for.
	...
	private void showBottomInformation(int what) {
		...
		TextView tvInfo = (TextView) activity
	...
	public void showUseHelpConfirmation(int option) {
		...
		TextView tvConfirmation = (TextView) activity
	...
	private void chargeLastGame() {
		if (MainActivity.isStarted == true) {
			Settings set = new Settings(mContext);
	...
	private void determineQuestionPointsValue() {
		 ...
		 * question value:
		 */
		int tempPoints = curLevel * (remainedTime / 10);
	...
	// To delete after, it is called in QuizActivity, in abandon() method:
	public void info() {
		Cursor c = mDbHelper
				.queryData("SELECT count(*) FROM intrebari WHERE LENGTH(var1)=1");
		int total = c.getInt(0);
		GUITools.toast("" + total, 100, mContext);
	} // end info() method.
